...
>();
  ...
  const getMovieCategory = () => {
    ...
    api
      .get(
        `${movieId}?filterByFormula=AND(%7BType%7D+%3D+'Series'%2C%7BLatest%7D+%3D+'Yes')&maxRecords=5`,
      )
      .then((response) =>
        response.data.records.length > 0
          ? (setSeries(response.data.records),
            setLoading(false))
          : (setLoading(false),
            setErrorMsg(
              `ooops!!!! our Tv-series catalogue is empty check back later`,
            )),
      )
      .catch(
        (error) => (
  ...
  const loadMore = (amount: number) => {
    setLoadingMore(true)
    api.get(`${movieId}?sort%5B0%5D%5Bfield%5D=Date&sort%5B0%5D%5Bdirection%5D=desc&filterByFormula=AND(%7BTYpe%7D+%3D+'Series'%2C%7BLatest%7D+%3D+'No')&maxRecords=${loadNumber + amount}`)
